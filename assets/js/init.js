...
$(document).ready(function() {
	...
	// init countdown
	if($("#countdown-widget").data('time') != '') {
		launchTime = new Date(Date.parse($("#countdown-widget").data('time')));
	}else{
...
function _resize() {
	...
	if(!_isIpad()) {
		...
		if( (winWidth <= 480 && prevWidth > 480) || (winWidth > 480 && prevWidth <= 480)) {
	...
	if(!_isMobile()) {
		...
		if(contact_form_open)
			$('#contact_form').css({height:winHeight - headerHeight});
	}else{
